...
def lots_personal_data_found(summary_df: pd.DataFrame, out_path: str):
    save_df(summary_df[summary_df["# personal data"] >= 10][["processed img"]], out_path)

def address_not_entirely_found(summary_df: pd.DataFrame, out_path: str):
    save_df(summary_df[summary_df["city"] != summary_df["city"]][["processed img"]], out_path)

def name_not_entirely_found(summary_df: pd.DataFrame, out_path: str):
    save_df(summary_df[summary_df["name_family"] != summary_df["name_first_0"]][["processed img"]], out_path)
